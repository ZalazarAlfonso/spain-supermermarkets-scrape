#!/usr/bin/env python3
"""
Al Campo (compraonline.alcampo.es) supermarket product scraper.

- Reads category targets from a JSON file (produced by targets_weekly)
- For each category URL, uses Playwright with infinite-scroll to capture all products
...
Notes:
- Al Campo is a React+Redux SPA that loads products via infinite scroll.
- fetch_scrolled() handles the scroll loop; no pagination BFS needed.
- Override group URLs with ALCAMPO_GROUP_URLS (comma-separated).
"""
...
def scrape_category(
    ...
    """Scrape all products from a single Al Campo category page using infinite scroll."""
    ...
    if not products:
        # Fallback path: parse a final HTML snapshot if progressive collection fails.
        try:
            ...
            print(
                ...
                f"cards={len(cards)} (fallback-html)"
            )
            if not cards:
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=os.getenv("TARGETS_GCS_OBJECT_NAME", "alcampo/al_campo_categories.json"),
    ...
    csv_filename = f"alcampo_supermercado_{today}.csv"
    parquet_filename = f"alcampo_supermercado_{today}.parquet"
    ...
    object_name = f"alcampo/{today}/{parquet_filename}"
