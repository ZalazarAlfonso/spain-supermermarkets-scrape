"""HTTP utilities for Lidl Spain requests."""
...
def fetch_text(session: requests.Session, url: str, timeout: int = REQUEST_TIMEOUT_S) -> str:
...
def fetch_json(session: requests.Session, url: str, timeout: int = REQUEST_TIMEOUT_S) -> Dict[str, Any]:
    """Fetch and parse a JSON object response."""
    resp = session.get(url, timeout=timeout, allow_redirects=True)
    resp.raise_for_status()
    data = resp.json()
